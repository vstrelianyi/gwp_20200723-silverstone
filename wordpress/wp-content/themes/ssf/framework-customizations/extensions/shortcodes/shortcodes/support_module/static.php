...
if(!class_exists('SupportQuestions')){
    ...
    class SupportQuestions {
        ...
        {
            if($product){
                $this->group_list = array_merge( $this->db_get_product_questions($product), $this->db_get_general_questions_for_product() );
                $this->spa = true;
        ...
        public function render_qa_group($single_group , $active = false, $echo = true, $product = null){
            $url = $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
            if(!empty($single_group)) {
                if (strpos($url,'pid=') !== false) {
                    $html = '<div class="support-block__tab-content" id="'.(!empty($single_group['group_id']) ? $single_group['group_id'] : '').'">';
                }
                else {
        ...
        public function render_single_list() {
            foreach ( $this->group_list as $key => $single_group ) {
                $this->render_single_group_link($single_group, $key == 0);
            }
        }

        public function render_single_group_link( $single_group , $active = false, $echo = true){
            if(!empty($single_group)){

                $html = '<a href="javascript:;" data-toggle="tab" data-target="#'.(!empty($single_group['group_id']) ? $single_group['group_id'] : '').'">';
                $html .= '<i class="'.(!empty($single_group['icon']) ? $single_group['icon'] : '').'"></i>';
                $html .= '<span>'.(!empty($single_group['name']) ? $single_group['name'] : '').'</span>';
                $html .= '<i class="icon-chevron"></i>';
                $html .= '</a>';
        ...
        public function get_general_questions_for_product(){
            if(empty($this->general_questions)){
                $this->general_questions = $this->db_get_general_questions();
            }
            return $this->general_questions;
        ...
        public function db_get_general_questions_for_product()
        {
            $raw_data = fw_get_db_settings_option('support_questions',[]);
            ...
            foreach ($raw_data as $group_key => $single_group) {
                if ($single_group['cat_name'] == 'Общие вопросы') {
                    $unified_group['group_id']   = '_gg-'.$group_key;
                    $unified_group['type']   = 'general';
                    $unified_group['name'] = !empty($single_group['cat_name']) ? $single_group['cat_name'] : '';
                    $unified_group['icon'] = !empty($single_group['icon']['icon-class']) ? $single_group['icon']['icon-class'] : '';
                    // echo $single_group['cat_name'];
